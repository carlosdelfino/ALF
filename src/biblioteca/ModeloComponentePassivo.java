/*
 * ModeloComponentePassivo.java Criado em 17/01/2004
 *
 * Todo Codigo criado abaixo, pode ser reutilizado para qualquer fim
 * desde que seja para o bem da humanidade e não seja para fins lucra-
 * tivos capitalistas.
 * 
 */
package biblioteca;


/**
 * @author Carlos Delfino
 * mail suporte@carlosdelfino.eti.br
 * site http://www.carlosdelfino.eti.br
 * 
 * Digite aqui o que esta classe faz.
 * Nome: 
 */
public class ModeloComponentePassivo extends ModeloComponente {
	ModeloComponentePassivo(){
		setCategoria(0);
	}

}
